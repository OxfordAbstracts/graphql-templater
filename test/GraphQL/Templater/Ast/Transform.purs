  ...
  describe "GraphQL.Templater.Ast.Transform" do
    describe "insertTextAt" do
      ...
      it "should insert into an each" do
        parseAndTest "a" 15 "{{#each list}}  {{/each}}" "{{#each list}} a {{/each}}"

parseAndTest :: forall m. MonadEffect m => MonadThrow Error m => String -> Int -> String -> String -> m Unit
parseAndTest insert idx input expected =
  case parse input, parse expected of
    Right inputParsed, Right expectedParsed -> do
      -- Console.log $ show inputParsed
      -- let result = insertTextAt insert idx inputParsed
